...
public abstract class DataboxRecipeProvider extends RecipeProvider {

	public DataboxRecipeProvider(PackOutput output) {
		super(output);
	}
	...
	public void ore(ItemLike result, List<ItemLike> ingredients, float xp, String group,
	...
	private void oreSmeltingRecipe(ItemLike result, List<ItemLike> ingredients, float xp, String group,
			Consumer<FinishedRecipe> consumer) {
		for (ItemLike ingredient : ingredients) {
			smeltingRecipe(result, ingredient, xp, 1).group(group).save(consumer, new ResourceLocation(ExampleMod.MOD_ID,
					"smelt_" + ForgeRegistries.ITEMS.getKey(ingredient.asItem()).getPath()));
		}
	}
	...
	public SimpleCookingRecipeBuilder smeltingRecipe(ItemLike result, ItemLike ingredient, float exp, int count) {
		return SimpleCookingRecipeBuilder
				.smelting(Ingredient.of(new ItemStack(ingredient, count)), RecipeCategory.MISC, result, exp, 200)
	...
	private void oreBlastingRecipe(ItemLike result, List<ItemLike> ingredients, float xp, String group,
			Consumer<FinishedRecipe> consumer) {
		for (ItemLike ingredient : ingredients) {
			blastingRecipe(result, ingredient, xp, 1).group(group).save(consumer, new ResourceLocation(ExampleMod.MOD_ID,
					"blast_" + ForgeRegistries.ITEMS.getKey(ingredient.asItem()).getPath()));
		}
	}

	public SimpleCookingRecipeBuilder blastingRecipe(ItemLike result, ItemLike ingredient, float exp, int count) {
		return SimpleCookingRecipeBuilder
				.blasting(Ingredient.of(new ItemStack(ingredient, count)), RecipeCategory.MISC, result, exp, 100)
				.unlockedBy("has_" + ForgeRegistries.ITEMS.getKey(ingredient.asItem()), has(ingredient));
	}
	...
	public SimpleCookingRecipeBuilder smokingRecipe(ItemLike result, ItemLike ingredient, float exp, int count) {
		return SimpleCookingRecipeBuilder
				.smoking(Ingredient.of(new ItemStack(ingredient, count)), RecipeCategory.MISC, result, exp, 100)
				.unlockedBy("has_" + ForgeRegistries.ITEMS.getKey(ingredient.asItem()), has(ingredient));

	}

	public SmithingTransformRecipeBuilder smithingRecipe(Supplier<Item> input, Supplier<Item> upgradeItem,
			Supplier<Item> templateItem, Supplier<Item> result) {
		return SmithingTransformRecipeBuilder
				.smithing(Ingredient.of(templateItem.get()), Ingredient.of(input.get()), Ingredient.of(upgradeItem.get()),
	...
	public SingleItemRecipeBuilder adamantiumStonecutting(ItemLike result) {
