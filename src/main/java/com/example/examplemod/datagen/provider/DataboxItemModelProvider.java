...
public abstract class DataboxItemModelProvider extends ItemModelProvider {

	public DataboxItemModelProvider(PackOutput output, ExistingFileHelper fileHelper) {
	...
	public ItemModelBuilder blockLamp(Supplier<? extends Block> block) {
		String offName = blockName(block) + "_off";
		String onName = blockName(block) + "_on";

		var offVariant =  withExistingParent(blockName(block), modLoc("block/" + offName));
		var onVariant =  withExistingParent(blockName(block), modLoc("block/" + onName));
	...
	public void trapdoor(Supplier<? extends TrapDoorBlock> trapdoor) {
		withExistingParent(ForgeRegistries.BLOCKS.getKey(trapdoor.get()).getPath(),
				new ResourceLocation(ExampleMod.MOD_ID, "block/" + blockName(trapdoor) + "_bottom"));
	}
}